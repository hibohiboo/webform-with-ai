...
- Q: フロントエンドのエラーメッセージ表示方式 → A: インライン表示（各入力欄の下にエラーメッセージ）
- Q: 日付入力欄の初期値 → A: 今月の1日〜本日をデフォルト設定
- Q: バックエンド 400 エラー時のレスポンス形式 → A: `{ "error": "ERROR_CODE", "message": "..." }` 形式
- Q: 未来の日付の許容 → A: 許可する（結果が空なら 204 No Content）
- Q: 204 No Content 時のフロントエンド表示 → A: 通知メッセージを表示（「指定期間にデータがありません」）
- Q: 日付フィルタのタイムゾーン解釈 → A: 日本時間（JST）として解釈し、バックエンドで UTC 変換して比較
...
1. **Given** フィードバックデータが複数日にわたって存在する, **When** from=2026-01-01&to=2026-01-31 でダウンロード, **Then** 2026年1月（JST）のフィードバックのみが CSV に含まれる
2. **Given** フィードバックが 2026-01-15 23:59:59.999 JST（= 14:59:59.999 UTC）に登録されている, **When** to=2026-01-15 でダウンロード, **Then** そのフィードバックが CSV に含まれる（境界値テスト）
3. **Given** フィードバックが 2026-01-15 00:00:00.000 JST（= 2026-01-14 15:00:00.000 UTC）に登録されている, **When** from=2026-01-15 でダウンロード, **Then** そのフィードバックが CSV に含まれる（境界値テスト）
...
- 未来の日付を指定した場合: 許可する（該当データがなければ 204 No Content）

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: システムは CSV ダウンロード API でクエリパラメータ `from` を受け付け、指定日の 00:00:00.000 JST（= 前日 15:00:00.000 UTC）以降のデータをフィルタする
- **FR-002**: システムは CSV ダウンロード API でクエリパラメータ `to` を受け付け、指定日の 23:59:59.999 JST（= 同日 14:59:59.999 UTC）までのデータをフィルタする
...
- **FR-007a**: 400 エラーレスポンスは `{ "error": "ERROR_CODE", "message": "説明文" }` 形式とする
  - `INVALID_DATE_FORMAT`: 日付フォーマット不正（YYYY-MM-DD 以外）
  - `INVALID_DATE`: 無効な日付（例: 2026-02-30）
- **FR-008**: フィルタ結果が空の場合、204 No Content を返す（既存動作を維持）

### Frontend Requirements

- **FR-009**: CSV ダウンロード画面に「開始日」「終了日」の入力欄を追加する
- **FR-010**: フロントエンドでは両方の日付入力が必須（未入力時はダウンロードボタンを無効化）
- **FR-011**: 開始日 > 終了日の場合、インラインでエラーメッセージを表示し、ダウンロードボタンを無効化する
- **FR-012**: フロントエンドで基本的な入力バリデーション（日付フォーマット、有効な日付）を行う
- **FR-013**: バリデーションエラー時は API リクエストを送信しない
- **FR-014**: エラーメッセージは該当する入力欄の直下にインラインで表示する
- **FR-015**: 画面表示時の初期値は「開始日: 今月1日」「終了日: 本日」とする
- **FR-016**: API が 204 No Content を返した場合、「指定期間にデータがありません」と通知メッセージを表示する

### Key Entities

- **DateRange**: 日付範囲を表現するパラメータペア（from, to）。各日付は YYYY-MM-DD 形式の文字列（JST として解釈）として受け取り、バックエンドで UTC 時刻範囲に変換される
...
- 日付パラメータ（YYYY-MM-DD）は日本時間（JST = UTC+9）として解釈する
- バックエンドでは JST を UTC に変換してから `submittedAt`（ISO 8601 UTC 形式）と比較する
